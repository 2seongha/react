...
const Detail: React.FC = () => {
  ...
  useLayoutEffect(() => {
    ...
    // 임시로 화면 밖에서 측정
    const tempElement = expandedHeaderRef.current.cloneNode(
      true
    ) as HTMLElement;
  ...
  const opacityRev = useTransform(
    scrollY,
    [halfHeight, expandedHeight],
    [0, 1]
  );
  const reversedY = useTransform(scrollY, (y) => Math.max(-y, -expandedHeight));
      ...
      >
        ...
        <motion.div
          style={{
            position: "fixed",
            height: "76px",
            top: `calc(var(--ion-safe-area-top) + ${expandedHeight}px + 48px)`,
            translateY: reversedY,
            width: "100%",
            zIndex: 2,
          }}
          className="grab-wrapper"
        >
          <div
            ref={grabRef}
            style={{
            ...
            <div className="grap-indicator-wrapper">
              <span
                style={{
                  ...
                  width: "60px",
                  height: "4px",
                  background: "var(--grab-indicator-color)",
                }}
              />
            </div>
            <IonSegment
          ...
          onSliderFirstMove={() => {
            ...
            if (swiper) {
              ...
              swiper.slides.forEach((slide, index) => {
                ...
                slide.scrollTop =
                  activeSlide.scrollTop < expandedHeight
                    ? 0
                    : slide.scrollTop < expandedHeight ? expandedHeight : slide.scrollTop;
              });
            }
          }}
          onTransitionStart={() => {
            ...
            const swiper = swiperRef.current;
            if (swiper) {
              const activeSlide = swiper.slides[swiper.previousIndex];
              if (!activeSlide) return;
              swiper.slides.forEach((slide, index) => {
                if (swiper.previousIndex === index) return;
                slide.scrollTop =
                  activeSlide.scrollTop < expandedHeight
                    ? 0
                    : slide.scrollTop;
        ...
        >
          <SwiperSlide
            style={{
              ...
              paddingBottom:
                P_AREA_CODE === "TODO"
                  ? "calc(var(--ion-safe-area-bottom) + 75px)"
          ...
          >
            <div
              style={{
                height: "auto",
                minHeight: `calc(100% + ${expandedHeight}px)`,
                background: `linear-gradient(to bottom, transparent 0px, transparent ${
                  expandedHeight + 76
                }px, var(--ion-background-color) ${
                  expandedHeight + 76
                }px, var(--ion-background-color) 100%)`,
              }}
            >
              <div
                style={{
                ...
                (sub: any, index: number) => (
                  <SubItem
                    ...
                    onAttendeeDialogOpen={(attendeeData) => {
                      ...
                      // 숨김 버튼을 클릭해서 Dialog 열기
                      document
                        .getElementById("attendee-dialog-trigger")
                        ?.click();
                    }}
                  />
                )
          ...
          >
            ...
            >
              <div
                style={{ height: expandedHeight, scrollSnapAlign: "start" }}
              ></div>
              <div
                style={{
                  scrollSnapAlign: "start",
              ...
              >
                {[...approval.LISTAPPRLINE, ...approval.LISTAPPRLINE].map(
                  (apprLine: any, index: number) => {
        ...
        {P_AREA_CODE === "TODO" && (
          <div
            style={{
              position: "fixed",
              bottom: 0,
              height: "calc(75px + var(--ion-safe-area-bottom))",
              width: "100%",
              backgroundColor: "var(--ion-background-color)",
              zIndex: "1",
